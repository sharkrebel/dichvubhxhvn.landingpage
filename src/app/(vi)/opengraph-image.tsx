...
export default async function Image() {
    try {
        ...
        // Fallback: Load static image directly
        // User requested simplification: "xuất ra file ảnh load cho đơn giản"
        const imageUrl = "https://dichvubhxh.vn/og-image-static.png";
        const res = await fetch(imageUrl);

        if (!res.ok) {
            // Absolute worst case fallback if fetch fails: simple text
            return new ImageResponse(
                <div style={{ background: '#0f172a', width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: 48 }}>
                    DichVuBHXH.vn
                </div>,
                { ...size }
            );
        ...
        return new Response(arrayBuffer, {
            headers: {
                ...
                "Cache-Control": "public, max-age=86400, transform-max-age=86400",
            },
        });
    }
}
